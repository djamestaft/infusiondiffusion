...
6. Before creating any task branch, fetch the latest remote `main` and require that fetch to succeed: run `git fetch <remote> main`, then create the branch with `git switch -c agent/<task-slug> <remote>/main`. This remote-tracking branch is the canonical fresh base; do not branch from a stale local `main`. Pulling the primary checkout is unnecessary and may disturb unrelated local work, but if a user explicitly requires local `main` to move too, first confirm it is clean and fast-forward it non-interactively. Never share a task branch or checkout between writing agents.
