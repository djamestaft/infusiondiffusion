...
10. **Bounded Codex connector only** — Pi agents must never invoke Codex directly (including path-qualified or wrapped commands). The sole exception is `adw_modules/codex_worker.py`, a deterministic code phase configured for exact read-only official Figma evidence; it has no repo write, merge, publish, deployment, or rollback authority. Follow `cookbooks/codex-connector-worker.md`.
11. **Every ADW ends in `run.finish()`** — phases passing is not the same as the run being accepted. A test phase that ran a red suite succeeded at its job. Pass `accepted=` so the exit code, the session status, and the banner are decided together and cannot disagree.
