...
The 2026-08 Foundations + Atoms implementation contract is approved as seven separate exact targets: [review contract `403:3`](https://www.figma.com/design/GYiQd7QSAwCSaGtt0alKG2/Infusion-Diffusion-Designs-WEB?node-id=403-3), [color roles `403:4`](https://www.figma.com/design/GYiQd7QSAwCSaGtt0alKG2/Infusion-Diffusion-Designs-WEB?node-id=403-4), [typography `403:5`](https://www.figma.com/design/GYiQd7QSAwCSaGtt0alKG2/Infusion-Diffusion-Designs-WEB?node-id=403-5), [space and shape `403:6`](https://www.figma.com/design/GYiQd7QSAwCSaGtt0alKG2/Infusion-Diffusion-Designs-WEB?node-id=403-6), [interaction and motion `403:7`](https://www.figma.com/design/GYiQd7QSAwCSaGtt0alKG2/Infusion-Diffusion-Designs-WEB?node-id=403-7), [implementation contract `455:3`](https://www.figma.com/design/GYiQd7QSAwCSaGtt0alKG2/Infusion-Diffusion-Designs-WEB?node-id=455-3), and [release QA gate board `456:2`](https://www.figma.com/design/GYiQd7QSAwCSaGtt0alKG2/Infusion-Diffusion-Designs-WEB?node-id=456-2). These targets refine the existing system; they do not replace approved page or Gallery contracts.

Runtime-only behavior remains deliberately test-owned: 320px and 390px mobile, tablet, desktop, wide desktop, natural-height and 200% zoom checks; keyboard and visible focus; WCAG AA contrast; reduced motion; loading/error/empty/disabled and long/localized content; image alternative text and rights applicability. Static Figma evidence cannot prove those behaviors.
...
## Gallery page — approved editorial viewer

## 2026-08-10 storefront composition synchronization

The approved responsive page compositions are now expressed directly in the runtime templates rather than only through foundations and shell states. Home uses the approved first-viewport editorial split, oversized Marcellus proposition, full-height campaign media, and image-overlay carousel controls. Collection uses a decisive title/lead introduction, tonal browsing canvas, and effect-free base-surface ProductCards with continuous off-white content fields. Product detail uses a full-height media/details split with a sticky desktop image and an uninterrupted mobile reading order. Fragrance Guide/Editorial, About, and Contact use the approved large-scale introductory hierarchy and asymmetric reading grids. Cart reuses the same editorial scale and a Midnight order-summary field while preserving Shopify-owned totals, availability, and checkout handoff.

The Gallery composition, authored order, media cadence, viewer behavior, and Sanity contract are intentionally unchanged. The shared Navigation, semantic tokens, Storybook component names, focus treatment, reduced-motion behavior, and public component APIs remain synchronized with their existing approved sources.

| Layer                               | Status                     | Evidence / divergence                                                                                                                    |
| ----------------------------------- | -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| Approved Figma responsive templates | synced                     | Exact Home, Collection, Product detail, Editorial, About, Contact, Gallery, and atomic targets remain recorded above and in feature docs |
| `DESIGN.md` and semantic CSS        | synced                     | Existing semantic roles retained; no raw replacement palette or new primitive introduced                                                 |
| Runtime page components             | synced                     | Home, Collection, Product detail, Editorial, About, Contact, ProductCard, HeroCarousel, and Cart compositions updated                    |
| Storybook                           | synced                     | Existing responsive, state, long-content, loading, empty, and commerce stories exercise the revised contracts                            |
| Gallery                             | intentional preservation   | Approved layout and data/viewer boundaries remain unchanged                                                                              |
| Production content and commerce     | pending owner verification | Sanity retains editorial ownership and Shopify retains all price, inventory, cart, and checkout truth                                    |

`/gallery` is governed by approved populated desktop/mobile [`357:3`](https://www.figma.com/design/GYiQd7QSAwCSaGtt0alKG2/Infusion-Diffusion-Designs-WEB?node-id=357-3)/[`357:34`](https://www.figma.com/design/GYiQd7QSAwCSaGtt0alKG2/Infusion-Diffusion-Designs-WEB?node-id=357-34), viewer [`357:64`](https://www.figma.com/design/GYiQd7QSAwCSaGtt0alKG2/Infusion-Diffusion-Designs-WEB?node-id=357-64)/[`357:76`](https://www.figma.com/design/GYiQd7QSAwCSaGtt0alKG2/Infusion-Diffusion-Designs-WEB?node-id=357-76), empty [`357:88`](https://www.figma.com/design/GYiQd7QSAwCSaGtt0alKG2/Infusion-Diffusion-Designs-WEB?node-id=357-88)/[`357:100`](https://www.figma.com/design/GYiQd7QSAwCSaGtt0alKG2/Infusion-Diffusion-Designs-WEB?node-id=357-100), and loading [`357:111`](https://www.figma.com/design/GYiQd7QSAwCSaGtt0alKG2/Infusion-Diffusion-Designs-WEB?node-id=357-111)/[`357:131`](https://www.figma.com/design/GYiQd7QSAwCSaGtt0alKG2/Infusion-Diffusion-Designs-WEB?node-id=357-131) frames. It is an Experience-mode composition: a compact Sage introduction yields to a borderless, authored-order image grid. The 1280px desktop field uses two 600px columns and the 350px mobile field collapses to one column; the tall/short cadence never changes DOM or keyboard order. Captions remain visible below images.

`GalleryViewer` composes the existing Dialog and Button primitives as a full Deep Ink viewport rather than a centered card. It contains media, starts focus on Close, restores focus to the exact thumbnail, supports Escape, provides written `Image N of M` context, and disables rather than wraps multi-item boundaries; a one-item viewer omits navigation. It uses existing light/dark semantic variables, 4px media/control corners, no card shadow or border, `next/image` responsive sizing, thumbnail hotspot placement, and contained viewer images. Gallery captions are authored `editorialPage.sections[].body` values; the existing Studio requirement remains authoritative, while malformed legacy missing values use the exact visible fallback “Additional details are unavailable for this image.” Final media requires rights confirmation and factual alternatives. Empty/unavailable and loading states stay honest and non-live; fixed Figma heights never override natural wrapping, zoom, or safe areas. Runtime deliberately preserves the complete five-link Navigation plus account/cart and corrects the static empty-frame collision.

The approved Gallery content extension is recorded in content-direction node [`376:2`](https://www.figma.com/design/GYiQd7QSAwCSaGtt0alKG2/Infusion-Diffusion-Designs-WEB?node-id=376-2) and market layout nodes [`377:2`](https://www.figma.com/design/GYiQd7QSAwCSaGtt0alKG2/Infusion-Diffusion-Designs-WEB?node-id=377-2), [`377:9`](https://www.figma.com/design/GYiQd7QSAwCSaGtt0alKG2/Infusion-Diffusion-Designs-WEB?node-id=377-9), and [`377:16`](https://www.figma.com/design/GYiQd7QSAwCSaGtt0alKG2/Infusion-Diffusion-Designs-WEB?node-id=377-16). Four portrait Campaign stories remain first in their approved cadence. At the approved 1440px desktop width, the independent left stack is Blanc De Blanc then Botanical with a 64px same-column gap; the right stack is Emerald then Library with a 32px same-column gap; every Campaign caption begins 16px below its image. Mobile preserves authored order in one column. The visible H2 `In the Market` introduces a separate five-item documentary grid: three fluid columns from 1024px until the fixed approved desktop composition fits at 1408px, with the first image spanning two columns, then one authored-order column below `lg`. Its media sequence is 16:9, 3:4, then three 4:3 slots with Sanity hotspot/crop protecting people, products, reeds, packaging, signage, and context. Campaign titles are H2s; market titles are H3s. Each grid owns an independent viewer count and the closing statement follows both. Either group may render alone; only neither group uses the existing empty state.
...
## Account entry — approved hosted Shopify handoff

`/account` is governed by the approved Account Review board [`337:321`](https://www.figma.com/design/GYiQd7QSAwCSaGtt0alKG2/Infusion-Diffusion-Designs-WEB?node-id=337-321). It reuses Ivory Navigation, ContentHeader, Button, TextLink, FeedbackAlert, and existing semantic tokens for hosted, loading, unavailable, not-provisioned, provider-error, and long-content states. The hosted action is a same-tab Shopify HTTPS destination; unavailable states offer real recovery links and never imitate a disabled action. Navigation intentionally has no Account-current variant in this approved slice; the H1 supplies page identity. This is an intentional documented divergence, not a new design-system contract.
