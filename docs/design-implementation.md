...
Devon owns INF-33, INF-34 and INF-35 implementation. Shawnee owns design/content
preparation and coordinates independent verification under INF-36. Start with
INF-33's reconciliation of the existing Shop/product delivery; do not rebuild
fixes already evidenced. INF-34 follows, then INF-35 subject to its remaining
Contact, Guide matching and content dependencies. Preserve the roadmap's gates.

Use one delivery branch by default. Each bounded delivery needs acceptance
criteria, exact design references, independent review and a human merge.
This consolidation does not start storefront implementation.
...
versions or abstractions with no use. The existing TemplateShell composes navigation and page content; implement
the approved shared footer with an explicit reusable contract and story
coverage during INF-34. Home and Shop share the approved ProductCard treatment
...
configuration and CI were inspected. Runtime and Storybook synchronization to
the revised designs remains the work of INF-33/34/35.
