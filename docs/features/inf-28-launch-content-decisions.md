...
## Recorded deferral — 7 September 2026

The user instructed: “we will add the fragrance notes later, lets just continue”.
Santuaire Serein’s specific notes are deferred and do not block remaining
preparation. Keep its existing supplied description unchanged and leave named
notes unlisted. The proposed short “Scent character” replacement was not
explicitly approved and must not be substituted. Do not infer ingredients or
add placeholder notes. U3 remains open for the other five note-list decisions;
U4 remains open for any description rewrite. Track the Santuaire follow-up in
INF-31, with Shopify remaining the source owner.

## Outstanding decision matrix

Devon confirmed “Only the reed diffusors for now” on 7 September 2026 in response
to the six existing 200 ml reed diffusers versus candles/room sprays question.
U2 is **Accepted**: advertise reed diffusers only for this launch. U1's range is
**Accepted**; its title suffix and Shopify product-type value remain **Unresolved**.
The Santuaire notes portion of U3 is **Deferred by user**. All other decisions
remain **Unresolved**. The [wording preview](evidence/inf-31-reed-diffuser-copy-preview.md)
maps this decision to its content sources; source publication is still separate.
“Recommendation” is preparation
...
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| U1  | Six titles have ` - 200ml`; `productType` is empty and all variants expose only `Title: Default Title`. Home calls them reed diffusers. A1/A3 already approve names and size.                                  | Accepted 7 September: these six 200 ml reed diffusers are the launch range. Recommend `<canonical name> - 200ml` as Shopify title to preserve the current size convention; confirm `Reed diffuser` as product type.                                                                             | Shopify `Product.title`, `productType`; existing `variants[].selectedOptions` remains untouched unless a separately approved option change is required.                                                                                                                                                                                           |
| U2  | Home and Fragrance Guide mention candles and room sprays; the observed catalogue contains only these six products.                                                                                             | Accepted by Devon 7 September: reed-diffuser-only launch messaging; candles and room sprays are excluded for now. Exact replacement copy is prepared in the INF-31 wording preview; publication remains separate.                                                                               | Sanity `siteSettings.introduction`, `seoDescription`, `homepage.heroIntroduction`; `editorialPage[slug.current == "fragrance-guide"].sections[].body`; matching repository fallbacks in `src/sanity/types.ts`, `editorial-pages.ts`, template defaults; internal `PRODUCT.md` purpose if approved scope changes.                                  |
| U3  | Five descriptions list explicit notes; Santuaire Serein does not.                                                                                                                                              | Confirm the five existing note lists; add Santuaire Serein's authoritative notes later (user-deferred; not a preparation blocker). Select 2–3 differentiating notes per product for a short summary in INF-31. Do not add a scent from imagery or another product.                              | Shopify product description (Storefront read field `description`; editable Admin description body), optionally `seo.description`. No new metafield is assumed. Sanity guide may link the product GID, not become the product-description owner.                                                                                                   |
...
The user authorized continuing preparation while Santuaire notes are deferred.
The [INF-31 preparation package](inf-31-launch-content-preparation.md) covers
approved naming and source-file inventory; other unapproved facts remain open.
Completion and publication still require their applicable gates. Its source
