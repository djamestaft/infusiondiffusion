...
| Local evidence                     | synced | sanitized 1440/390/320 fixture screenshots, command record, and authenticated Preview release record                                          |

## Provisioning and release gates

A null destination is a valid external-provisioning state until a Shopify owner enables/configures customer accounts and the vanity domain. Authenticated Preview release review completed for exact commit `f7b5129`: the `quality` check, Vercel deployment, and `just pr-gate 47` passed; `/api/health` reported that revision; and desktop plus 347×605 `/account` screenshots showed the approved safe unavailable state, no local account UI, no handoff action, and no exposed destination. The reviewed console contained no displayed error and the observed response headers retained `noindex` handling.

Only the sanctioned sanitized flag-state observation (valid HTTPS versus null, without retaining a URL) and Preview keyboard/axe checks remain pending. Those checks must not authenticate to Shopify or record a destination. Independent review, human merge, and Production enablement remain closed gates. The first human-authorized rollback is setting `SHOPIFY_ACCOUNT_HANDOFF_ENABLED=false`, redeploying, and confirming the unavailable state.
