...
- All four templates provide approved sticky Navigation treatments in both modes. Light mode uses Sage Base `#EEF0E7` and the Antique Gold bottom divider represented by Figma instance `157:171`; Midnight uses the dark surface and corresponding divider represented by instance `157:182`. Mobile uses the matching closed light or Midnight MobileNavigation variant. Borderless navigation is not part of the template contract. The `/shop` Collection browsing canvas uses semantic Content Surface Elevated (`#E3E7DA` in Ivory, `#191916` in Midnight) while ProductCards retain their base surface and no border or shadow; this tonal role does not alter Navigation, Product detail, or Editorial surfaces.
- Template canvases and ordinary inner layout sections use semantic `color/background/base`, which resolves to approved Sage Base `#EEF0E7` in light mode. Selected feature fields use Sage Quiet `#DDE2D4` through `color/background/subtle`. This lowers glare while preserving distinction between the canvas, navigation, cards, and selected content fields; inner sections must not mask the canvas with hardcoded white.
...
- The ordinary page canvas uses the semantic `content-surface` token (Sage Base `#EEF0E7`); the selected fragrance-guidance band uses Sage Quiet `#DDE2D4`. Navigation owns its light or Midnight mode independently of the page canvas.
