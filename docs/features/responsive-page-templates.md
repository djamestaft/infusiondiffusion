...
- All four templates provide approved Navigation treatments in both modes. Ivory uses the light surface and gold bottom divider represented by Figma instance `157:171`; Midnight uses the dark surface and corresponding divider represented by instance `157:182`. Mobile uses the matching closed Ivory or Midnight MobileNavigation variant. Borderless navigation is not part of the template contract.
...
- Product photography remains provisional. About/Contact route integration, final content, analytics, homepage expansion, and production asset replacement remain pending.
