...
- All four templates provide approved sticky Navigation treatments in both modes. Light mode uses Sage Base `#EEF0E7`, continuous with the page canvas, and the Antique Gold bottom divider represented by Figma instance `157:171`; Midnight uses the dark surface and corresponding divider represented by instance `157:182`. Mobile uses the matching closed light or Midnight MobileNavigation variant. Borderless navigation is not part of the template contract.
