...
13. Codex is the project agent coordinator. Before delegating, tell the user which roles will run, what each owns, and where human approval is required. Keep implementation on one delivery branch by default; create multiple writing agents, worktrees, or pull requests only when the user approves that topology.
