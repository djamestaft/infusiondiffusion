...
## 2026-08-09 Foundations + Atoms synchronization addendum

This addendum implements the approved atomic foundation contract without migrating pages or changing Gallery, Sanity, Shopify, or public component APIs. The seven exact targets were captured separately and approved by `design-authority`; runtime-only assertions remain owned by Storybook, Vitest, Playwright, and Preview QA.

| Figma target                    | Approved evidence                                                   | DESIGN / sidecar / CSS                                                | Component API                                                   | Storybook / test evidence                                     | Status         | Divergence                                                                                      |
| ------------------------------- | ------------------------------------------------------------------- | --------------------------------------------------------------------- | --------------------------------------------------------------- | ------------------------------------------------------------- | -------------- | ----------------------------------------------------------------------------------------------- |
| `403:3` Review contract         | Exact node, `Approved`, eight handoff categories                    | Exact-node contract recorded in `DESIGN.md` and `foundationRefresh`   | Existing exports, props, aliases and native semantics preserved | `Foundations/Approved atoms`; token and full gates            | synced         | Runtime behavior is test-owned because static Figma cannot prove it                             |
| `403:4` Color roles             | Ivory and Midnight semantic roles                                   | Existing semantic pairs retained; mode mappings asserted              | All atoms continue to consume semantic tokens                   | Ivory/Midnight foundation stories; token tests                | synced         | No intentional visual divergence                                                                |
| `403:5` Typography              | Marcellus expressive roles; Manrope operational roles               | Existing display/headline/title/body/label contract retained          | Heading levels remain semantic and independent from treatment   | Typography specimens and heading assertion                    | already synced | No third display voice introduced                                                               |
| `403:6` Space and shape         | 4px scale, disciplined 4px controls, 8px surfaces, restrained depth | Stable spacing, radius, depth, gutter and content-max variables added | Component size/variant strings unchanged                        | Space/shape/depth specimens; deterministic token assertions   | synced         | Existing full rounding remains only where geometry is circular/pill-like                        |
| `403:7` Interaction and motion  | Visible focus, state motion, reduced-motion obligation              | Mode-complete focus and 150ms state tokens retained                   | Keyboard, disabled, busy and Radix behavior unchanged           | Focus specimen, atom stories, reduced-motion story and tests  | synced         | Runtime behavior is test-owned                                                                  |
| `455:3` Implementation contract | Exact node and bounded provenance                                   | Figma → docs → sidecar → CSS → Storybook crosswalk complete           | No breaking API or page migration                               | Focused/full verification commands below                      | synced         | Components absent from a specific static frame remain unchanged                                 |
| `456:2` Release QA board        | 1440×1540 gate board, `Approved`                                    | Viewports cover 320, 390, tablet, 1440 and wide desktop               | Gallery and page contracts remain immutable                     | Storybook viewports; Gallery and broad browser gates required | synced         | Natural-height, 200% zoom, image alt/rights and content extremes require runtime/owner evidence |

### Verification and residual release work

- Deterministic local evidence is recorded in the delivery handoff and judged by command exit status.
- Image alternative text and rights applicability remain content-owner obligations for every production asset; this change introduces no imagery.
- The Gallery implementation and its authored order, cadence, captions, viewer boundaries, Escape behavior and focus restoration are unchanged.
- No roadmap update is required: this is the explicitly approved Foundations + Atoms slice.
- Protected CI, Vercel Preview screenshots/health, final content and asset approval, human merge approval, and separate production authorization remain pre-go-live gates.
