...
function fixture() {
  document.body.innerHTML = `
    <div id="outgoing"><h1><span>O</span><span>L</span><span>D</span></h1></div>
    <div id="incoming">
...
it("starts the outgoing letters left before overlapping the right-side arrival", () => {
  ...
  timeline.progress(1);
  expect(Number(gsap.getProperty(outgoingChars[0], "x"))).toBe(-24);
...
it("caps the stagger for long headlines so the CTA does not wait for every letter", () => {
  ...
  expect(timeline.duration()).toBeLessThanOrEqual(0.85);
  timeline.pause().seek(0.72);
  expect(
